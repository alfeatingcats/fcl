...
function VisuallyHiddenInput<T = InputValue>(
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      ...
      if ("borderBoxSize" in entry) {
        ...
        const borderSize = Array.isArray(borderSizeEntry)
          ? (borderSizeEntry[0] as ResizeObserverSize)
          : (borderSizeEntry as unknown as ResizeObserverSize);
  ...
  React.useEffect(() => {
    ...
    if (prevValue !== currentValue && descriptor?.set) {
...
function parseColorString(value: string): ColorValue | null {
  ...
  const hsbMatch =
    /^hsba?\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([\d.]+))?\s*\)$/.exec(
      trimmed,
    );
  if (hsbMatch) {
